            ...
            void Absorb(AuraEffect* aurEff, DamageInfo& dmgInfo, uint32& absorbAmount)
            {
                ...
                if (victim->GetTypeId() == TYPEID_UNIT/* && victim->ToCreature()->IsNPCBot()*/)
                {
                    if (remainingHealth <= 0 && !victim->ToCreature()->GetSpellHistory()->HasCooldown(PAL_SPELL_ARDENT_DEFENDER_HEAL))
                    {
                        ...
                        extra.SpellValueOverrides.AddMod(SPELLVALUE_BASE_POINT0, healAmount);
                        extra.SpellValueOverrides.AddMod(SPELLVALUE_BASE_POINT1, NULL);
                        extra.SpellValueOverrides.AddMod(SPELLVALUE_BASE_POINT2, NULL);
                        extra.CastItem = NULL;
