...
void WorldSession::HandleMoveSplineDoneOpcode(WorldPacket& recvData)
{
	...
	//TC_LOG_DEBUG("lasyan3.automount", "mvc=%d spell=%d", player->m_mountCanceled, player->m_mountSpell);
    if (player->m_mountCanceled && player->m_mountSpell > 0)
    {
        player->CastSpell(player, player->m_mountSpell, true);
		TC_LOG_DEBUG("lasyan3.automount", "AutoMount casted from HandleMoveSplineDoneOpcode");
    }
