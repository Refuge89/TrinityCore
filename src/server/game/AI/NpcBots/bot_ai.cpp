...
bool bot_ai::doCast(Unit* victim, uint32 spellId, bool triggered, ObjectGuid originalCaster)
{
    ...
    if (spellId == MANAPOTION)
    {
        ...
        extra.SpellValueOverrides.AddMod(SPELLVALUE_BASE_POINT0, value);
        extra.SpellValueOverrides.AddMod(SPELLVALUE_BASE_POINT1, 0);
        extra.SpellValueOverrides.AddMod(SPELLVALUE_BASE_POINT2, 0);
        me->CastSpell(victim, spellId, extra);
        return true;
    }
        ...
        switch (enchant_display_type)
        {
            ...
            case ITEM_ENCHANTMENT_TYPE_EQUIP_SPELL:
                if (enchant_spell_id)
                {
                    ...
                    if (basepoints)
                    {
                        extra.SpellValueOverrides.AddMod(SPELLVALUE_BASE_POINT0, basepoints);
                        extra.SpellValueOverrides.AddMod(SPELLVALUE_BASE_POINT1, basepoints);
                        extra.SpellValueOverrides.AddMod(SPELLVALUE_BASE_POINT2, basepoints);
                        me->CastSpell(me, enchant_spell_id, extra);
                    } 
                    else
                        me->CastSpell(me, enchant_spell_id, extra);
                }
...
void bot_minion_ai::ApplyItemEquipSpell(Item* item, bool apply)
{
    ...
    for (uint8 i = 0; i != MAX_ITEM_PROTO_SPELLS; ++i)
    {
        ...
        if (apply) {
            ...
            extra.TriggerFlags = TriggerCastFlags::TRIGGERED_FULL_MASK;
            extra.CastItem = item;
            me->CastSpell(me, spellInfo->Id, extra);
        }
        else
    ...
    if (procVictim & PROC_FLAG_TAKEN_DAMAGE)
    {
        for (uint8 i = 0; i != MAX_ITEM_PROTO_SPELLS; ++i)
        {
            ...
            if (roll_chance_f(chance)) {
                ...
                extra.TriggerFlags = TriggerCastFlags::TRIGGERED_FULL_MASK;
                extra.CastItem = item;
                me->CastSpell(target, spellInfo->Id, extra);
            }
        }
    }
